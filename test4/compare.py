...
dataframe1 = pd.DataFrame(data=processed["Normal"], columns=columnsofdt)
drqn = dataframe1['SCORE']

dataframe2 = pd.DataFrame(data=processed["Random"], columns=columnsofdt)
random = dataframe2['SCORE']

dataframe3 = pd.DataFrame(data=processed["Replay"], columns=columnsofdt)
dqn = dataframe3['SCORE']

plt.axhline(linewidth=1, color='r')

comparison1 = addComparison(drqn, random, "NormalvsRandom", "orange")
comparison2 = addComparison(dqn, random, "ReplayvsRandom", "green")
printComparison(comparison1, "NormalvsRandom")
printComparison(comparison2, "ReplayvsRandom")
